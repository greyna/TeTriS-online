...
function Map() { // Map des pièces déposées sous forme matricielle (1 si cube présent, 0 sinon)
	...
	this.draw = function() {
		...
		for (var i=0; i < 22; i++)
			for (var j=1; j < 31; j++)
				if (him.matrice[i][j]) ctx.fillRect((j-1)*20,i*20,20,20);
	};
}
...
function Piece() { // class Piece
  ...
  this.moveToBot = function (period) {
	...
	if (moveR) {
		moveR=0;
		if (map.collisionHorizontale(him)!==1) him.xp+=20;	// translation Right d'un cube (+20px)
	}
	else if (moveL) { // si l'utilisateur va à gauche et à droite en même temps, ben il ira à droite et ça bugera pas :p
		moveL=0;
		if (map.collisionHorizontale(him)!==-1) him.xp-=20;	// translation Left d'un cube (-20px)
	}
	if (turn) {
		turn=0;
		...
		if ((map.collisionHorizontale(him)!==0)) { // dans le cas où tourner impliquerait une collision
